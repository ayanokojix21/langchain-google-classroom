...
class TestExecuteWithRetry:
    ...
    @patch("time.sleep")
    def test_custom_base_delay(self, mock_sleep: MagicMock) -> None:
        """Custom base_delay should affect sleep duration."""
        HttpError = _make_http_error_class()
        ...
        with (
            ...
            execute_with_retry(mock_request, max_retries=3, base_delay=5.0)

        # First retry: base_delay * (2^0) + jitter = 5.0 + 0.5 = 5.5
        mock_sleep.assert_called_once()
        actual_delay = mock_sleep.call_args[0][0]
        assert actual_delay == pytest.approx(5.5)
