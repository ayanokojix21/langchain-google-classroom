...
class TestGetParser:
    ...
    def test_text_csv_returns_csv_parser(self) -> None:
...
class TestCSVParser:
    """Tests for CSVParser (BaseBlobParser interface)."""

    def test_basic_csv(self) -> None:
        """Basic CSV produces structured output with headers."""
        from langchain_google_classroom.parsers.csv_parser import CSVParser

        csv_data = b"Name,Grade,Subject\nAlice,95,Math\nBob,87,Science\n"
        blob = Blob(data=csv_data, mimetype="text/csv")
        parser = CSVParser()
        docs = list(parser.lazy_parse(blob))

        assert len(docs) == 1
        assert "Spreadsheet (2 rows):" in docs[0].page_content
        assert "Columns: Name, Grade, Subject" in docs[0].page_content
        assert "Row 1: Name: Alice | Grade: 95 | Subject: Math" in docs[0].page_content
        assert "Row 2: Name: Bob | Grade: 87 | Subject: Science" in docs[0].page_content
        assert docs[0].metadata["row_count"] == 2
        assert docs[0].metadata["column_count"] == 3
        assert docs[0].metadata["headers"] == ["Name", "Grade", "Subject"]

    def test_empty_csv(self) -> None:
        """Empty CSV yields nothing."""
        from langchain_google_classroom.parsers.csv_parser import CSVParser

        blob = Blob(data=b"", mimetype="text/csv")
        parser = CSVParser()
        docs = list(parser.lazy_parse(blob))
        assert docs == []

    def test_header_only_csv(self) -> None:
        """CSV with only headers yields a doc with 0 rows."""
        from langchain_google_classroom.parsers.csv_parser import CSVParser

        blob = Blob(data=b"Name,Grade\n", mimetype="text/csv")
        parser = CSVParser()
        docs = list(parser.lazy_parse(blob))

        assert len(docs) == 1
        assert "Spreadsheet (0 rows):" in docs[0].page_content
        assert docs[0].metadata["row_count"] == 0

    def test_bom_handling(self) -> None:
        """BOM at start of CSV is stripped."""
        from langchain_google_classroom.parsers.csv_parser import CSVParser
        ...
        assert docs[0].metadata["headers"] == ["Name", "Score"]

    def test_empty_row_filtering(self) -> None:
        """Fully empty rows are filtered out."""
        from langchain_google_classroom.parsers.csv_parser import CSVParser
    ...
    def test_truncation(self) -> None:
        """Large CSV is truncated with summary."""
        from langchain_google_classroom.parsers.csv_parser import CSVParser

        lines = ["Name,Score"]
        for i in range(20):
            lines.append(f"Student{i},{90 + i}")
        csv_data = "\n".join(lines).encode()

        blob = Blob(data=csv_data, mimetype="text/csv")
        parser = CSVParser(max_inline_rows=5)
        docs = list(parser.lazy_parse(blob))

        assert len(docs) == 1
        assert "showing first 5" in docs[0].page_content
        assert "15 more rows omitted" in docs[0].page_content
        assert docs[0].metadata["truncated"] is True
        assert docs[0].metadata["row_count"] == 20

    def test_no_row_numbers(self) -> None:
        """Row numbers can be disabled."""
        from langchain_google_classroom.parsers.csv_parser import CSVParser

        csv_data = b"Name,Score\nAlice,95\n"
        blob = Blob(data=csv_data, mimetype="text/csv")
        parser = CSVParser(include_row_numbers=False)
        docs = list(parser.lazy_parse(blob))

        assert len(docs) == 1
        assert "Row 1:" not in docs[0].page_content
        assert "Name: Alice | Score: 95" in docs[0].page_content

    def test_is_base_blob_parser(self) -> None:
        """CSVParser is a BaseBlobParser subclass."""
        from langchain_google_classroom.parsers.csv_parser import CSVParser

        assert issubclass(CSVParser, BaseBlobParser)
